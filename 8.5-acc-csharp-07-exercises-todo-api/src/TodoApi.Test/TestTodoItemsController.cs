...
public class TestTodoItemsController : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        var response = await _client.PostAsync(path, new StringContent(jsonToAdd, System.Text.Encoding.UTF8, "application/json"));
        response.StatusCode.Should().Be(expectedStatusCode);
    }
    ...
    {
        var response = await _client.PutAsync(path, new StringContent(jsonToAdd, System.Text.Encoding.UTF8, "application/json"));
        response.StatusCode.Should().Be(expectedStatusCode);
    }
