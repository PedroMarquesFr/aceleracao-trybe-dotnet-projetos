...
public class TestTrybeGamesDatabase
{
    ...
    {
        var gamesByPlayers = databaseEntry.GetGamesPlayedBy(databaseEntry.Players[0]);
        gamesByPlayers.Should().BeEquivalentTo(expected);
    }
    ...
    {
        var gamesByPlayers = databaseEntry.GetGamesOwnedBy(databaseEntry.Players[0]);
        gamesByPlayers.Should().BeEquivalentTo(expected);
    }
    ...
    {
        var selectedGames = databaseEntry.GetGamesDevelopedBy(databaseEntry.GameStudios[0]);
        selectedGames.Should().BeEquivalentTo(expected);
    }
